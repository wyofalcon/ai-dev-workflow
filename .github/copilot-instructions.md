...
## 📝 Post-Builder SESSION.md Update Protocol
...
**The Builder is NOT allowed to modify SESSION.md** — it's in their protected files list. Only the Auditor writes session history.

---

## 🤖 Auditor Role: Monitoring & Automation
...
**Routing Rules:**
- **Code generation tasks** → Send refined prompt to `builder`
- **Shell commands** (git, docker, npm, tests) → Send to `shell`
- **File edits, searches, analysis** → Use built-in tools directly (no terminal needed)
...
|--------|------------|---------------|
| Create branch | 🟢 Low | Auto-execute |
| Stage changes | 🟢 Low | Auto-execute |
| Commit changes | 🟢 Low | Auto-execute |
| Push to feature branch | 🟢 Low | Auto-execute |
| Run tests/linting | 🟢 Low | Auto-execute |
| Apply labels | 🟢 Low | Auto-execute |
| Open pull request | 🟡 Medium | Auto-execute (notify user) |
| Merge to dev | 🟡 Medium | Request approval |
| Merge to main | 🔴 High | **REQUIRE explicit approval** |
| Delete branch | 🔴 High | **REQUIRE explicit approval** |
| Force push | 🔴 High | **REQUIRE explicit approval** |
...
3. If high-risk → Pause and ask: "I detected [signals]. Recommended action: [action]. Approve? (yes/no)"
4. If low-risk → Execute and report result

**Example low-risk auto-execution:**
```
📊 Workflow Status: Ready for commit
🔍 Signals: User said "looks good", all tests pass, no uncommitted changes in other files
🎯 Action: Committing changes to feature/easy-cv branch
✅ Executed: git commit -m "feat(easy-cv): implement floating wizard UI"
```
...
🔍 Signals: PR approved, tests passing, "ready to merge" detected
🎯 Recommended: Merge feature/easy-cv → main
❓ This requires your approval. Proceed? (yes/no)
```
